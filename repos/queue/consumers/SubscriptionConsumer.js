...
const billingHistoryRepo = require('../../BillingHistoryRepo');
...
const chargingAttemptRepo = require('../../ChargingAttemptRepo');
        ...
        if (subscription.active === true) {
            ...
            if (subscription.amount_billed_today < config.maximum_daily_payment_limit_pkr ) {
                ...
                if (countThisSec < config.telenor_subscription_api_tps) {
                    ...
                    setTimeout(() => {
                        console.log("Calling consume subscription queue after 500 seconds");
                        consume(message);
                    }, 200);
                }  
            }else{
                console.log("Excessive charging");
                await subscriptionRepo.markSubscriptionInactive(subscription._id);
...
// CHARGING ATTEMPTS
tryFullChargeAttempt = async(queueMessage, subscription, transaction_id, is_manual_recharge) => {
    try{
        let packageObj = await packageRepo.getPackage(subscription.subscribed_package_id);
        let user = await userRepo.getUserBySubscriptionId(subscription._id);
        let response = await billingRepo.fullChargeAttempt(user.msisdn, packageObj, transaction_id);
        ...
        if(message === 'Success'){
            ...
            let updatedSubscription = await subscriptionRepo.updateSubscription(subscription._id, subscriptionObj);
...
tryDiscountedChargeAttempt = async(queueMessage, subscription, transaction_id, discounted_price) => {
    try{
        let packageObj = await packageRepo.getPackage(subscription.subscribed_package_id);
        ...
        if(message === 'Success'){
            ...
            // Send acknowledgement message
            await chargingAttemptRepo.resetAttempts(subscription._id);
            await chargingAttemptRepo.markInActive(subscription._id);
            await chargingAttemptRepo.unqueue(subscription._id);
...
tryMicroChargeAttempt = async(queueMessage, subscription, transaction_id, micro_price) => {
    try{
        let packageObj = await packageRepo.getPackage(subscription.subscribed_package_id);
        let user = await userRepo.getUserBySubscriptionId(subscription._id);
        let response = await billingRepo.microChargeAttempt(user.msisdn, packageObj, transaction_id, micro_price);
...
// ASSIGN GRACE PERIOD
assignGracePeriod = async(subscription, user, packageObj, is_manual_recharge) => {
    ...
    if((subscription.subscription_status === 'billed' || subscription.subscription_status === 'trial') && subscription.auto_renewal === true){
        ...
        messageRepo.sendSmsToUser(message, user.msisdn);

        let attempt = await chargingAttemptRepo.getAttempt(subscription._id);
        if(attempt && attempt.active === false){
            await chargingAttemptRepo.markActive(subscription._id);
            await chargingAttemptRepo.resetAttempts(subscription._id);
...
unQueue = async(subscription_id) => {
    await subscriptionRepo.updateSubscription(subscription_id, {queued: false})
}

addMicroChargingToQueue = async(subscription) => {
    let attempt = await chargingAttemptRepo.getAttempt(subscription._id);
    if(attempt){
        await chargingAttemptRepo.incrementAttempt(subscription._id);
        ...
        await chargingAttemptRepo.createAttempt({subscription_id: subscription._id, number_of_attempts_today: 1});    
        console.log('Created Charging Attempt Record - Subscription ', subscription._id);
    }
}

// SHOOT EMAIL
shootExcessiveBillingEmail = async(subscription_id) => {
...
sendAffiliationCallback = async(tid, mid, user_id, subscription_id, subscriber_id, package_id) => {
