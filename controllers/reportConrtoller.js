...
exports.billing_stats = async (req,res) =>  {
    ...
    data.push({'Yesterdays success requests till the time': requests[0].sum});
    data.push({'Yesterdays failed requests till the time': requests[1].sum});

    requests = await billingHistoryRepo.getBillingStats(dayBeforeYesterdayStart, dayBeforeYesterdayEnd);
    data.push({'Day before yesterdays success requests till the time': requests[0].sum});
    data.push({'Day before yesterday failed requests till the time': requests[1].sum});
